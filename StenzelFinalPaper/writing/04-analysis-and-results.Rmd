...
for(warm in warm_grid){
  n.warm = floor(n.iter* warm/10) # throw away warm_grid * 10 percent
  theta_str = paste("theta", as.character(n.warm), sep="")
...
#sdf["theta"] = sdf$ind
ggplot(sdf, aes(x=ind, y=values)) + 
  geom_dotplot(binaxis='y', stackdir='center') + scale_x_discrete(labels= floor(n.iter* warm_grid/10)) + xlab("Warmup") + ylab(TeX("$\\hat{\\theta}_{USA, 2008}$")) + scale_colour_brewer(palette="Blues") + grids(linetype = "dashed")
