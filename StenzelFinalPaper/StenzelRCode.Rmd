...
\noindent
Verbally, this means that the probability distribution in the next step does not change. Thus, stationarity is a sufficient condition for the statement hat $pi*$ in some period $t$ has converged. It is, however, a weaker statement than Aperiodicity, as it does not require that the "probability flux" out of state $i$ goes into step $i$ in the next period but only that the probability of state $i$ does not change over time independent from which state the "probability flux" arrives.

These three definitions are enough to understand the next fundamental theorem.

### Basic Limit Theorem

The next theorem defines formally the condition when a Markov Chain converges to a unique stationary distribution.

\begin{theorem}
If a Markov chain is irreducible and aperiodic, then:\\
\noindent
1. There exists a stationary distribution $\pi^*$\\
\noindent
2. $\lim_{t \rightarrow \infty} P(X_t = i) = \pi_i^*, \forall i$
\end{theorem}

\noindent
Therefore, if we want to construct a stable distribution $p(\theta)$ via Markov chains, we need to ensure that it is irreducible and aperiod with stationary distribution $\pi^*=p(\theta)$.
